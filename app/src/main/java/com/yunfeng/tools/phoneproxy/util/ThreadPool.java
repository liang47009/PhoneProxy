...
public class ThreadPool {
    ...
    public interface Job<T> {
        T run(JobContext jc) throws Exception;
    }
    ...
    public ThreadPool(int initPoolSize, int maxPoolSize) {
        mExecutor = new ThreadPoolExecutor(initPoolSize, maxPoolSize, KEEP_ALIVE_TIME, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), new PriorityThreadFactory("thread-pool", android.os.Process.THREAD_PRIORITY_BACKGROUND));
    }
    ...
    private class Worker<T> implements Runnable, Future<T>, JobContext {
        ...
        private Worker(Job<T> job, FutureListener<T> listener) {
            mJob = job;
            mListener = listener;
        }
